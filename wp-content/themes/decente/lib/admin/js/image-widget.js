(function($){
	...
	function image_widget_send_to_editor(h) {
		...
		$( '#widget-'+self.IW_instance+'-height' ).val($( '#display-widget-'+self.IW_instance+'-image img').attr('width'));
		...
		// change button text 
		$('#add_image-widget-'+self.IW_instance+'-image').text(frameshift_spaces.frameshift_change_image);
		
		// change preview layer class
		$('#display-widget-'+self.IW_instance+'-image').removeClass('no-image');
		
		// display image settings
		$('#settings-widget-'+self.IW_instance+'-image').removeClass('no-image');		
	...
	function changeImgHeight(instance) {
		var height = $( '#widget-'+instance+'-width' ).val();
