...
if ( is_admin() ) {
	...
	$frameshift_page = 'appearance_page_frameshift';
	add_action( "admin_print_styles-$frameshift_page", 'frameshift_mlu_css', 0 );
	add_action( "admin_print_scripts-$frameshift_page", 'frameshift_mlu_js', 0 );
	
	add_action( "admin_print_styles-widgets.php", 'frameshift_mlu_css', 0 );
	add_action( "admin_print_scripts-widgets.php", 'frameshift_mlu_js', 0 );	
}
...
if ( ! function_exists( 'frameshift_mlu_init' ) ) {
	function frameshift_mlu_init () {
		register_post_type( 'frameshift', array(
			'labels' => array(
				'name' => __( 'Options Framework Internal Container', 'frameshift' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'frameshift_mlu_css' ) ) {

	function frameshift_mlu_css () {
...
if ( ! function_exists( 'frameshift_mlu_js' ) ) {

	function frameshift_mlu_js () {
	
		// Registers custom scripts for the Media Library AJAX uploader.
		wp_register_script( 'of-medialibrary-uploader', FRAMESHIFT_ADMIN_URL .'/js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
 ...
 *
 * Dependencies:
 * - frameshift_mlu_get_silentpost()
 */

if ( ! function_exists( 'frameshift_medialibrary_uploader' ) ) {

	function frameshift_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$int = frameshift_mlu_get_silentpost( $id );
		...
		$output .= '<div class="screenshot ' . $class . '" id="' . $id . '_image">' . "\n";
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'frameshift' ) . '" rel="' . $int . '" />' . "\n";
		
		if ( $_desc != '' ) {
			$output .= '<span class="frameshift_metabox_desc">' . $_desc . '</span>' . "\n";
		}
		return $output;
	}	
}
 ...
 * Return: $_postid.
 *
 * If no "silent" post is present, one will be created with the type "frameshift"
 * and the post_name of "of-$_token".
 *
 * Example Usage:
 * frameshift_mlu_get_silentpost ( 'frameshift_logo' );
 */

if ( ! function_exists( 'frameshift_mlu_get_silentpost' ) ) {

	function frameshift_mlu_get_silentpost ( $_token ) {
	
		global $wpdb;
		$_id = 0;
	
		// Check if the token is valid against a whitelist.
		// $_whitelist = array( 'frameshift_logo', 'frameshift_custom_favicon', 'frameshift_ad_top_image' );
		// Sanitise the token.
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'frameshift', 'post_name' => 'of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
...
if ( ! function_exists( 'frameshift_mlu_insidepopup' ) ) {

	function frameshift_mlu_insidepopup () {
	
		if ( isset( $_REQUEST['is_frameshift'] ) && $_REQUEST['is_frameshift'] == 'yes' ) {
		
			add_action( 'admin_head', 'frameshift_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'frameshift_mlu_modify_tabs' );
		}
	}
}

if ( ! function_exists( 'frameshift_mlu_js_popup' ) ) {

	function frameshift_mlu_js_popup () {

		$_frameshift_title = $_REQUEST['frameshift_title'];
		if ( ! $_frameshift_title ) { $_frameshift_title = 'file'; } // End IF Statement
?>
	...
	jQuery(function($) {
		...
		$( 'h3.media-title' ).each ( function () {
			...
			var new_title = current_title.replace( 'media file', '<?php echo $_frameshift_title; ?>' );
			$( this ).html( new_title );
		...
		// Preserve the "is_frameshift" parameter on the "delete" confirmation button.
		$( '.savesend a.del-link' ).click ( function () {
			...
			continueHref = continueHref + '&is_frameshift=yes';
...
if ( ! function_exists( 'frameshift_mlu_modify_tabs' ) ) {

	function frameshift_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'frameshift' ), __( 'Previously Uploaded', 'frameshift' ), $tabs['gallery'] );
		return $tabs;
	}
}